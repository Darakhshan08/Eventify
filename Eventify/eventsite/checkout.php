...
if (isset($_POST['book'])) {
    ...
    $sql = "INSERT INTO tbl_booking(booking_id, user_id, event_id, no_member, user_remarks, booking_status) VALUES(:bkid, :userid, :eid, :noofmembers, :usrremark, :status)";
    ...
    if ($lastInsertId) {
        echo '<script>alert("Event booked successfully. Booking number is ' . $bkid . '")</script>';
    ...
    // Clear the temp_cart table
    $sql1 = "DELETE FROM temp_cart";
    $query2 = $conn->prepare($sql1);
    $query2->execute();

    $sqlEvent = "SELECT event_name FROM tbl_events WHERE id=:eid";
    ...
    try {
        ...
        $mail->Port = 465; // TCP p; // TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`
        ...
        $mail->Body = '<b>You ve acquired ' . $noofmembers . ' tickets for the ' . $eventname . ' event. </b>';
...
<section class="page-title bg-title overlay-dark">
  <div class="container">
    <div class="row">
      <div class="col-12 text-center">
        ...
        <div class="col-12"> <span>Card Number:</span>
                            <input type="tel" id="numericTelInput" name="numericTelInput" required>
        </div>
                            <div class="row">
    ...
    ?>
<input type="hidden" name="bkid" value="<?php echo htmlentities($row->booking_id);?>">
<input type="hidden" name="ubid" value="<?php echo htmlentities($row->user_id);?>">
<input type="hidden" name="evid" value="<?php echo htmlentities($row->event_id);?>">
<input type="hidden" name="noofmembers" value="<?php echo htmlentities($row->no_member);?>">
<input type="hidden" name="userremark" value="<?php echo htmlentities($row->user_remarks);?>">
<input type="hidden" name="stat" value="<?php echo htmlentities($row->booking_status);?>">
                        ...
                        <div class="row lower">
                            <div class="col text-left"><b>QUANTITY:</b></div>
                            <div class="col text-right"><b>x<?php echo htmlentities($row->no_member); ?></b></div>
                        </div>
                       
                        
                      
                        <div class="row lower">
                            <div class="col text-left"><b>PRICE:</b></div>
                            <div class="col text-right"><b>Rs <?php echo htmlentities($row->price); ?></b></div>
                        </div>
                        <div class="row lower">
                            <div class="col text-left"><b>Sub_Total:</b></div>
                            <div class="col text-right"><b>Rs <?php echo htmlentities($row->total); ?></b></div>
                        </div>
                       
                       
                        <button class="btn" type="submit" name="book" >Place order</button>
                        </form>

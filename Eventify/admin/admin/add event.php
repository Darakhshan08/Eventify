...
if (strlen($_SESSION['adminFullName']) == 0) {
    ...
    if (isset($_POST['add'])) {
        ...
        if (!in_array($extension, $allowed_extensions)) {
            ...
            // Query for insertion data into the database
            $sql = "INSERT INTO  tbl_events(cat_id,sponser_id,event_name,event_discription,event_start_date,event_end_date,event_location,event_image,is_active,price) VALUES(:catid,:spnserid,:ename,:ediscription,:esdate,:eedate,:elocation,:eventimage,:status,:price)";
            ...
            try {
                ...
                $mail->Body = '<b>The ' . $ename . ' event is now featured on our website. Secure your tickets immediately!</b>';
            ...
            <div class="col-lg-8 offset-lg-2">
                <form method="POST" enctype="multipart/form-data">
                    <div class="form-group">
                        ...
                        <select name="category" class="respond">
                    ...
                    <div class="form-group">
                        ...
                        <select class="respond" name="sponser">
                            <option>Select</option>
                            <?php
                            $sql = "SELECT s_id,sponsers_name from tbl_sponser";
                            $query = $conn->prepare($sql);
                            $query->execute();
                            $results = $query->fetchAll(PDO::FETCH_OBJ);
                            $cnt = 1;
                            if ($query->rowCount() > 0) {
                                foreach ($results as $row) {
                            ?>
                                    <option value="<?php echo htmlentities($row->s_id); ?>"><?php echo htmlentities($row->sponsers_name); ?></option>
                            <?php }
                            } ?>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" type="text" name="evetndescription">
                    </div>
                    <div class="form-group">
                        <label>Price</label>
                    ...
                    <div class="m-t-20 text-center">
                        <button class="btn btn-primary submit-btn" type="submit" name="add" style="background-color: #9a28d7;">Add Event</button>
                    </div>
                </form>
            </div>
        </div>

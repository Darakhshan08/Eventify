...
if(isset($_POST['add'])) {
    ...
    $newsd = $_POST['description']; // Corrected field name to 'description'
    $nlogo = $_FILES["NewsImage"]["name"];
    
    // get the image extension
    $extension = substr($nlogo, strlen($nlogo) - 4, strlen($nlogo));
    ...
    if (!in_array($extension, $allowed_extensions)) {
        ...
        $imgnewfile = md5($nlogo) . $extension; // Corrected variable name to 'nlogo'
        
        // Code for moving image into the directory
        move_uploaded_file($_FILES["NewsImage"]["tmp_name"], "newsimage/" . $imgnewfile);
        
        // Query for insertion data into the database
        $sql = "INSERT INTO  tbl_news(news_title, news_details, news_img) VALUES(:sponser, :newsd, :nlogo)";
        $query = $conn->prepare($sql);
        $query->bindParam(':sponser', $news, PDO::PARAM_STR);
        $query->bindParam(':newsd', $newsd, PDO::PARAM_STR);
        $query->bindParam(':nlogo', $imgnewfile, PDO::PARAM_STR);
        $query->execute();

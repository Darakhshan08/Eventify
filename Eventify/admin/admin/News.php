...
{
$id=$_GET['del'];
$sql = "delete from tbl_news  WHERE n_id=:id";
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['search_button'])) {
    ...
    $sql = "SELECT * FROM tbl_news WHERE news_title LIKE :search_query or news_details LIKE :search_query";
...
// Fetch all data
$sqlAll = "SELECT * FROM tbl_news LIMIT $recordsPerPage OFFSET $offset";
            ...
            <div class="content">
                <div class="row">
                    <div class="col-sm-4 col-3">
                        <h4 class="page-title" style="font-size: 40px;">Manage News</h4>
                    </div>
                    <div class="col-sm-8 col-9 text-right m-b-20">
                        <a href="add news.php" class="btn btn  btn-rounded float-right" style="background-color: #9a28d7; color: white;">Add News</a>
        ...
        <?php if ($showFetchedData): ?>
            <?php foreach ($allResults as $row): ?>
                                        ...
                                        <td><?php echo htmlentities($cnt);?></td>
                                            ...
                                            <td>
    <?php 
    $description = htmlentities($row->news_details);
    $maxLength = 60; // Define the maximum character limit for the description
    if (strlen($description) > $maxLength) {
        $truncatedText = substr($description, 0, $maxLength);
        echo $truncatedText . '... ';
        echo '<a href="#" data-toggle="modal" data-target="#descriptionModal' . $row->n_id . '">Read More</a>';
    } else {
        echo $description;
    }
    ?>
...
<div id="descriptionModal<?php echo $row->n_id; ?>" class="modal fade" role="dialog">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-body">
                <h4><?php echo htmlentities($row->news_title); ?></h4>
                <p><?php echo htmlentities($row->news_details); ?></p>
            </div>
        </div>
    </div>
</div>

                                            <td><img src="newsimage/<?php echo htmlentities($row->news_img);?>" width="120px" height="100px"></td>
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content"style="background: url(assets/img/stars.jpg);">
                        <div class="modal-body text-center">
                            ...
                            <a  class="btn btn-danger" href="News.php?del=<?php echo htmlentities($row->n_id);?>">Delete</a>
                            </div>
                        </div>
                    </div>
                </div>
